        ...
        try:
            ...
            for sheet_name in xl.sheet_names:
                ...
                for idx, row in df_data.iterrows():
                    ...
                    for attr in ['teacher_val', 'group_val', 'subject_val', 'wasnot_val']:
                        if locals()[attr].lower() == 'nan':
                            locals()[attr]  # read only — reassign below
                    if teacher_val.lower() == 'nan': teacher_val = ''
                    if group_val.lower()   == 'nan': group_val   = ''
                    if subject_val.lower() == 'nan': subject_val = ''
                    if wasnot_val.lower()  == 'nan': wasnot_val  = ''
    ...
    for file in files:
        try:
            ...
            for idx in range(data_start, len(df)):
                ...
                if ho_ten.lower()    == 'nan': ho_ten    = ''
                if don_vi.lower()    == 'nan': don_vi    = ''
                if bo_mon.lower()    == 'nan': bo_mon    = ''
                if doi_tuong.lower() == 'nan': doi_tuong = ''
...
def calculate_slots_lich_ky(df_lich_ky, from_date, to_date):
    """
    Tính tổng slot Lịch kỳ mỗi GV trong khoảng ngày.
    Lọc bỏ SlotTypeCode = 'G'.
    """
    mask       = (df_lich_ky['Date'] >= from_date) & (df_lich_ky['Date'] <= to_date)
    df_filtered = df_lich_ky[mask].copy()
...
def calculate_slots_fap(df_teaching_summaries, from_date, to_date):
    """
    Tính tổng slots FAP mỗi GV.
    Slots FAP = Σ từng lớp: (AllPlan - WasNotTaken)
    """
    ...
    df_ts = df_teaching_summaries.copy()
    df_ts['FromDate'] = df_ts['FromDate'].apply(parse_date_robust)
    df_ts['ToDate']   = df_ts['ToDate'].apply(parse_date_robust)

    mask       = (df_ts['FromDate'] == from_date) & (df_ts['ToDate'] == to_date)
    df_filtered = df_ts[mask].copy()

    if df_filtered.empty:
        overlap_mask = (
            df_ts['FromDate'].notna() &
            df_ts['ToDate'].notna() &
            (df_ts['FromDate'] <= to_date) &
            (df_ts['ToDate'] >= from_date)
        )
        df_filtered = df_ts[overlap_mask].copy()

    if df_filtered.empty and 'SourceMonth' in df_ts.columns:
        target_month = None
        if pd.notna(to_date):
        ...
        if target_month is not None:
            source_month = pd.to_numeric(df_ts['SourceMonth'], errors='coerce')
            df_filtered  = df_ts[source_month == target_month].copy()

    if df_filtered.empty:
        return pd.DataFrame(columns=['AccountFE', 'SlotsFAP'])

    def calc_slots(row):
        actual = row['AllPlan'] - row['WasNotTaken']
        return max(actual, 0)
...
def calculate_gio_dt(df_cham_cong, thang):
    ...
    return df_filtered[['ID', 'HoTen', 'DonVi', 'BoMon', 'DoiTuong', 'GioDayDT']].copy()
...
def doi_sanh_gio_day(df_lich_ky, df_teaching_summaries, df_cham_cong, df_gv_mapping):
    ...
    for _, month_row in months.iterrows():
        ...
        # Dùng ngưỡng thay vì so sánh == 0 để tránh lỗi floating-point
        master['KetQua'] = (
            (master['ChenhLech_LichKy_DT'].abs() < 0.001) &
    ...
    cols_order = [
        'Thang', 'ID', 'HoTen', 'AccountFE', 'DonVi', 'BoMon', 'DoiTuong',
    ...
    return final.sort_values(['Thang', 'HoTen']).reset_index(drop=True)
...
def get_wasnot_taken_detail(df_teaching_summaries):
    ...
    optional_cols = [c for c in ['SourceFile', 'SourceSheet', 'FromDate', 'ToDate'] if c in df_filtered.columns]
    ...
    ordered_cols += [c for c in ['SourceFile', 'SourceSheet', 'FromDate', 'ToDate'] if c in result.columns]
...
def calculate_gio_co_huu(df_lich_ky_full, df_cham_cong, df_gv_mapping):
    ...
    if 'ID' in df_gv_mapping.columns and 'AccountFE' in df_gv_mapping.columns:
        doi_tuong_mapping = doi_tuong_mapping.merge(
            df_gv_mapping[['ID', 'AccountFE']], on='ID', how='left'
        )
    ...
    result = doi_tuong_mapping.merge(gio_per_gv, on='AccountFE', how='left')
    result['TongGio'] = result['TongGio'].fillna(0)

    tong_gio_all     = result['TongGio'].sum()
    tong_gio_co_huu  = result[result['DoiTuong'].apply(is_co_huu)]['TongGio'].sum()
...
def build_lich_ky_kiem_tra(df_lich_ky_full, df_cham_cong, df_gv_mapping):
    """
    Tạo sheet lịch kỳ kiểm tra: thêm Tháng theo khoảng chấm công và
    thông tin GV link qua ID → AccountFE.
    """
    ...
    if 'ID' in df_gv_mapping.columns and 'AccountFE' in df_gv_mapping.columns:
        ...
        df_check = df_check.merge(
            ...
            left_on='Lecturer', right_on='AccountFE', how='left'
        )
    else:
    ...
    existing_cols = [c for c in cols_order if c in df_check.columns]
...
def generate_filename(base_name="KiemSoatGioDayGV"):
    """
    Tạo tên file theo định dạng yyyymmdd_tên.xlsx
    Ví dụ: 20260604_KiemSoatGioDayGV.xlsx
    """
    today = datetime.now().strftime("%Y%m%d")
    return f"{today}_{base_name}.xlsx"


def export_to_excel(df_doi_sanh, df_wasnot_taken, df_co_huu=None,
                    df_lich_ky_kiem_tra=None, base_name="KiemSoatGioDayGV"):
    """
    Xuất kết quả ra file Excel với nhiều sheet và định dạng chuyên nghiệp.
    Trả về (BytesIO, filename) với tên file dạng yyyymmdd_tên.xlsx
    """
    output = BytesIO()
    ...
    with pd.ExcelWriter(output, engine='openpyxl') as writer:
        ...
        if df_doi_sanh is not None and not df_doi_sanh.empty:
            df_doi_sanh.copy().to_excel(writer, sheet_name='Đối sánh giờ dạy', index=False)
        ...
        # Sheet 4: Lịch kỳ kiểm tra (nếu có)
    ...
    for sheet_name in wb.sheetnames:
        ...
        for row in range(2, max_row + 1):
            ...
            for col in range(1, max_col + 1):
                ...
                # Căn lề & định dạng theo kiểu cột
                if col_name in [
                    'Thang', 'ID', 'AccountFE', 'KetQua',
                    'CheckSlots_LichKy_FAP', 'FromDate', 'ToDate',
                    'Group', 'Subject', 'LaCoHuu', 'SoBuoiNghi'
                ]:
                    cell.alignment = align_center
                    ...
                    col_name and (
                        'Gio' in col_name or
                        'HeSo' in col_name or
                        'ChenhLech' in col_name
                    )
                ):
                    cell.alignment     = align_right
                    cell.number_format = '#,##0.00'  # ← sửa từ #,##0.0 → #,##0.00
                    try:
                        if val is not None:
                            cell.value = float(val)
                    except (ValueError, TypeError):
        ...
        for col in ws.columns:
            ...
            for cell in col:
                ...
                padding = 4 if cell.row == 1 else 0
                max_len = max(max_len, len(val_str) + padding)
